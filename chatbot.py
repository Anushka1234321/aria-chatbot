...
.stApp {
    background: radial-gradient(ellipse at 20% 20%, rgba(124,111,255,0.12), transparent 60%),
                radial-gradient(ellipse at 80% 80%, rgba(192,132,252,0.08), transparent 60%),
                #0c0b14;
}
...
.stChatInputContainer {
    border-top: 1px solid rgba(124,111,255,0.2) !important;
...
st.markdown("<p style='color: rgba(241,240,255,0.5); margin-top: -12px; font-size: 14px;'>Powered by Claude · Built with Python & Streamlit</p>", unsafe_allow_html=True)
st.divider()

# API Key input
api_key = st.sidebar.text_input("🔑 Enter your Anthropic API Key", type="password")
st.sidebar.markdown("[Get a free API key →](https://console.anthropic.com)", unsafe_allow_html=True)
st.sidebar.divider()
st.sidebar.markdown("**About this project**")
st.sidebar.markdown("An AI chatbot built with Python, Streamlit, and the Anthropic Claude API.")
            ...
            with st.spinner("Aria is thinking..."):
                try:
                    ...
                    response = client.messages.create(
                        ...
                        max_tokens=1000,
                        system="You are Aria, a helpful and friendly AI assistant. Be concise, warm, and genuinely useful.",
                        messages=[
                            {"role": m["role"], "content": m["content"]}
                            for m in st.session_state.messages
                            if m["role"] != "assistant" or m != st.session_state.messages[0]
                        ]
                    )
